...
const gym = {
  ...
  updateLift: (exerciseId, workingKg) => electron.ipcRenderer.invoke("store:lift", { exerciseId, workingKg }),
  completeSession: (payload) => electron.ipcRenderer.invoke("store:complete", payload),
  skipSession: (sessionId, sessionName) => electron.ipcRenderer.invoke("store:skip-session", { sessionId, sessionName }),
