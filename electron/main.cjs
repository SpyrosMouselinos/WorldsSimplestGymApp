...
function readState() {
  ...
  if (state.smtp?.encryptedPassword) {
    if (!safeStorage.isEncryptionAvailable())
      throw new Error('Windows credential storage is unavailable. Try reopening the app.');
    state.smtp = {
      user: state.smtp.user,
      appPassword: safeStorage.decryptString(Buffer.from(state.smtp.encryptedPassword, 'base64')),
    };
  }
  return state;
}
function writeState(state) {
  ...
  if (saved.smtp?.appPassword) {
    if (!safeStorage.isEncryptionAvailable())
      throw new Error('Windows credential storage is unavailable. Email settings were not saved.');
    saved.smtp = {
      user: saved.smtp.user,
      encryptedPassword: safeStorage.encryptString(saved.smtp.appPassword).toString('base64'),
    };
  }
  ...
  if (fs.existsSync(store.path)) {
    ...
    if (previous.smtp?.appPassword && safeStorage.isEncryptionAvailable()) {
      previous.smtp = {
        user: previous.smtp.user,
        encryptedPassword: safeStorage.encryptString(previous.smtp.appPassword).toString('base64'),
      };
...
function createWindow() {
  const win = new BrowserWindow({
    ...
    show: !process.env.GYM_TEST_DATA_DIR,
    webPreferences: {
...
if (!app.requestSingleInstanceLock()) app.quit();
  ...
  app
    ...
    .then(async () => {
      ...
      if (store.get('smtp')?.appPassword && safeStorage.isEncryptionAvailable())
      ...
      const { parseBackup } = await import(
        pathToFileURL(path.join(__dirname, '../src/planner.js')).href
      );
      handle('store:restore', (payload) => {
        return writeState({ ...parseBackup(payload), smtp: readState().smtp });
      });
      let sending = false;
      async function send(payload, test) {
        if (sending) return { ok: false, error: 'An email is already being sent.' };
        const state = readState();
        if (!state.smtp?.user || !state.smtp.appPassword)
          return {
            ok: false,
            error: 'Add a Gmail address and app password in Settings → Optional email first.',
          };
        if (
          !test &&
          (!payload ||
            ...
            payload.exercises.length > 30)
        )
          return { ok: false, error: 'Invalid workout email.' };
        sending = true;
        try {
          const recipient = state.profile.email || state.smtp.user;
          return await (test
            ? sendTestEmail(state.smtp, recipient)
            : sendWorkoutEmail(state.smtp, recipient, payload));
        } finally {
          sending = false;
        }
      }
      ...
      for (const action of ['min', 'max', 'close'])
        ipcMain.on(`win:${action}`, (event) => {
          trusted(event);
          const win = BrowserWindow.fromWebContents(event.sender);
